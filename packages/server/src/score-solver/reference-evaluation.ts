// Frozen benchmark opponent (2026-09-16). Keep independent of search/evaluation
// improvements so arena comparisons use the same opponent and objective.
...
export function potential(state: GameState, player: PlayerState, strategy = 0): number {
  ...
  return final + emphasis + Math.min(player.coins, 25) * (0.1 + remaining * 0.1)
