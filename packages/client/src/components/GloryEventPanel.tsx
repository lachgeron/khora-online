...
export const GloryEventPanel: React.FC<GloryEventPanelProps> = ({
  ...
  return (
    <div className="py-4">
      ...
      {!hasInteractive && (() => {
        // Build effect badges for the standings recap
        const recapEffects: Record<string, PlayerEffect[]> = {};
        for (const p of gameState.players) {
          const s = summaries[p.playerId];
          if (!s) continue;
          const effects = s.effects.filter(e => !e.endsWith('...'));
          if (effects.length === 0) continue;
          recapEffects[p.playerId] = effects.map(e => ({
            text: e,
            type: e.startsWith('+') ? 'gain' as const : e.startsWith('-') ? 'loss' as const : 'action' as const,
          }));
        }
        return (
          <>
